...
class Temp_receive_po_model extends CI_Model
{
  ...
  {
    $rs = $this->mc
    ->select('U_WEBORDER, LineNum, ItemCode, Dscription, BaseDocNum AS po_code, Quantity, FisrtBin AS BinCode')
    ...
    if(! $this->mc->where('DocEntry', $docEntry)->delete('PDN1'))
    {
      $sc = FALSE;
    }
    else
    {
      if( ! $this->mc->where('DocEntry', $docEntry)->delete('OPDN'))
      {
        $sc = FALSE;
      }
    }
